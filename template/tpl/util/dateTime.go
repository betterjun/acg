...
func GetTimeFromDefaultString(str string) time.Time {
	layout := "2020-11-23 11:59:28.579"
...
func FmtStr2YMDHMS(str string) string {
	...
	newTime, _ := time.ParseInLocation(layout, str, loc)
	day := newTime.Format(layout)
	return day
}
...
func GetTimeAfterMinute(start string, num int) time.Time {
	...
	d, _ := time.ParseDuration(allHourStr)
	nextDay := t.Add(d)
	//day := nextDay.Format("2006-01-02 15:04:05")
	return nextDay
}
